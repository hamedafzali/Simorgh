...
// This script runs from the backend process, so resolve backend dependencies
// from the backend package instead of the admin app.
const requireFromBackend = createRequire(
  path.resolve(__dirname, "../../../backend/package.json")
);
const sqlite3 = requireFromBackend("sqlite3").verbose();
const mongoose = requireFromBackend("mongoose");

// MongoDB Models
const Exam = require("../../../backend/database/models/Exam");
const Flashcard = require("../../../backend/database/models/Flashcard");
const Word = require("../../../backend/database/models/Word");
const seedDatabase = require("../../../backend/src/seed/seedData");

class SQLiteGenerator {
  constructor() {
    this.db = null;
    this.outputDir = path.join(__dirname, "../../dist/sqlite");
    this.ownsMongoConnection = false;
  }
  ...
  async ensureSourceData() {
    const before = await this.getSourceStats();
    const hasSourceData = Object.values(before).some((count) => count > 0);

    if (hasSourceData) {
      return { seeded: false, before, after: before };
    }

    console.log("Source MongoDB is empty. Running initial seed...");
    await seedDatabase();
    const after = await this.getSourceStats();
  ...
  async generateDatabase() {
    try {
      ...
      const source = await this.ensureSourceData();
      ...
      if (this.ownsMongoConnection && mongoose.connection.readyState === 1) {
